The Aho Corasick algorithm is a powerful structure that allows to perform operation in a set of words, it means in a dictionary.
The match operation has two types:
- The first is to count how many matches are in the string s;
- The second give the list of matches and sizes in every position
...
struct AhoCorasick{
  ...
  // Return a list of matches at each position in the string text
  // Can be used to verify if all the text is composed for word existing in the dictionary
